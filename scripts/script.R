...
# read data
cl_data <- read.csv(file = 'data/classification-data.csv')
...
# read data
pred_data <- read.csv(file = 'data/prediction-data.csv')
#View(pred_data)
...
sorted_data <- merged_data[order(merged_data$trial_index),]
#View(sorted_data)

# remove some columns
data <- sorted_data[-c(2,3,4,5,7,8)]
View(data)
...
pred_score <- (length(which(data$pred_correct == 1))/nrow(data))*100
